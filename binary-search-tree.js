...
class BinarySearchTree {
    ...
    insert (val) {
        // if there is no node add a node and a val,  and return it
        if(this.root === null) {
            this.root = new Node(val);
            return this;
        }

        // find the right spot for the new node
    ...
    insertRecursively (val, current = this.root) {
        // if the tree has nothing there put one at the root
        if(this.root === null) {
            this.root === new Node(val);
            return this;
        }

        // placing the nodes in the correct spot
        if(val < current.val) {
            if(current.left === null) {
                current.left = new Node(val);
                return this;
